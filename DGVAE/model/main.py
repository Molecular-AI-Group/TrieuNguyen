...
class DGE_Encoder(nn.Module) : 
    def __init__(self, dim_encoder, size_graph_vocab, dim_edge, num_head, num_layer, dropout_encoder, dropout_gat) : 
        ...
        for i in range(num_layer) :
            if i == 0 : conv = gnn.GATv2Conv(size_graph_vocab, dim_encoder//num_head, heads=num_head, dropout=dropout_gat, edge_dim=dim_edge)
            else : conv = gnn.GATv2Conv(dim_encoder, dim_encoder//num_head, heads=num_head, dropout=dropout_gat, edge_dim=dim_edge)
            norm = nn.BatchNorm1d(dim_encoder)
            act = nn.LeakyReLU(inplace=True)
            layer = gnn.models.DeepGCNLayer(conv, norm, act, block='res+', dropout=dropout_encoder)
...
class EncoderLayer(nn.Module) : 
    def __init__(self, dim, dim_ff, dim_edge, num_head, dropout_encoder, dropout_gat) : 
        super(EncoderLayer, self).__init__()
        self.dim = dim 

        self.norm1 = nn.BatchNorm1d(dim)
        self.attn1 =  gnn.GATv2Conv(dim, dim//num_head, heads=num_head, dropout=dropout_gat, edge_dim=dim_edge)
        self.drop1 = nn.Dropout(dropout_encoder)

        self.norm2 = nn.BatchNorm1d(dim)
        self.attn2 = gnn.GATv2Conv(dim, dim//num_head, heads=num_head, dropout=dropout_gat, edge_dim=dim_edge)
...
class Encoder(nn.Module) :
    def __init__(self, dim, dim_ff, dim_edge, size_graph_vocab, num_head, num_layer, dropout_encoder, dropout_gat) : 
        super(Encoder, self).__init__()
        self.layers = clones(EncoderLayer(dim, dim_ff, dim_edge, num_head, dropout_encoder, dropout_gat), num_layer)
        self.layers[0].attn1 = gnn.GATv2Conv(size_graph_vocab, dim//num_head, heads=num_head, dropout=dropout_gat, edge_dim=dim_edge)
